...
process Bracken {
    ...
    if [[ -f "${params.krakenuniq_bracken_dir}/\$KRAKEN_REPORT" && -f "${params.krakenuniq_bracken_dir}/\$CLASSIFIED" && -f "${params.krakenuniq_bracken_dir}/\$UNCLASSIFIED" ]]; then
        ...
        if [[ -f "${params.krakenuniq_bracken_dir}/\$BOUT_G" && -f "${params.krakenuniq_bracken_dir}/\$BOUT_S" ]]; then
            ...
            # Link/Copy
            for f in "\$KRAKEN_REPORT" "\$CLASSIFIED" "\$UNCLASSIFIED" "${SAMPLE_NAME}.bracken.G.report.txt" "${SAMPLE_NAME}.bracken.S.report.txt" "${SAMPLE_NAME}.bracken.G.krakenreport.txt" "${SAMPLE_NAME}.bracken.S.krakenreport.txt" "${SAMPLE_NAME}.bracken.G.mpa.krakenreport.txt" "${SAMPLE_NAME}.bracken.S.mpa.krakenreport.txt"; do
